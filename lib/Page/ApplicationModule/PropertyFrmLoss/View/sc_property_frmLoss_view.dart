...
class SCPropertyFrmLossView extends StatefulWidget {
  ...
  final SCPropertyFrmLossController state;

  SCPropertyFrmLossView({Key? key, required this.state}) : super(key: key);

  @override
  SCPropertyFrmLossViewState createState() => SCPropertyFrmLossViewState();
}

class SCPropertyFrmLossViewState extends State<SCPropertyFrmLossView> {
  ...
  Widget addItem() {
    return SCAddEntryButton(name: '新增报损', tapAction: () {
      SCRouterHelper.pathPage(SCRouterPath.addPropertyFrmLossPage, null);
    },);
  }
  ...
  detailAction(SCMaterialEntryModel model) {
    ...
    SCRouterHelper.pathPage(SCRouterPath.propertyFrmLossDetailPage, {'id': model.id, 'canEdit' : canEdit});
  }
