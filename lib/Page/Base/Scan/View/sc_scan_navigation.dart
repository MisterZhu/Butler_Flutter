...
class SCScanNavigation extends StatelessWidget {
  ...
  /// 返回按钮
  Widget backItem() {
    return button(icon: SCAsset.iconScanBack, onTap: () {
      tapBack?.call();
    });
  }
  ...
  /// 相册按钮
  Widget albumItem() {
    return button(icon: SCAsset.iconScanAlbum, onTap: () {
      tapAlbum?.call();
    });
  }
  ...
  /// 闪光灯按钮
  Widget flashItem() {
    return button(icon: isOpenFlash ? SCAsset.iconScanFlashOn : SCAsset.iconScanFlashOff, onTap: () {
        tapFlash?.call();
    });
  }
